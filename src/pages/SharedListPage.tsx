import { useMemo } from 'react';
import { useParams, Link, useNavigate } from 'react-router-dom';
import { useQuery } from '@tanstack/react-query';
import { collection, getDocs, query, where } from 'firebase/firestore';
import { db } from '@/lib/firebase';
import { useAuth } from '@/hooks/useAuth';
import { useShareInfo } from '@/hooks/useSharedList';
import { useWants, useAddWant, useRemoveWant } from '@/hooks/useWants';
import { Button } from '@/components/ui/button';
import { Skeleton } from '@/components/ui/skeleton';
import { buildWantId } from '@/types';
import type { Want, Release } from '@/types';

import rsdLogo from '@/images/rsd-logo.png';
import heartIcon from '@/images/heart.svg';

/** Fetch wants for a shared user (public read when sharingEnabled) */
function useSharedWants(uid?: string, eventId?: string | null) {
  return useQuery<Want[]>({
    queryKey: ['sharedWants', uid, eventId],
    enabled: !!uid,
    queryFn: async () => {
      if (!uid) return [];
      const wantsRef = collection(db, 'users', uid, 'wants');
      const q = eventId ? query(wantsRef, where('eventId', '==', eventId)) : query(wantsRef);
      const snap = await getDocs(q);
      return snap.docs.map((d) => ({ wantId: d.id, ...d.data() }) as Want);
    },
    staleTime: 30 * 1000,
  });
}

export default function SharedListPage() {
  const { shareId } = useParams<{ shareId: string }>();
  const navigate = useNavigate();
  const { isAuthenticated } = useAuth();
  const { data: shareInfo, isLoading: shareLoading, error: shareError } = useShareInfo(shareId);

  // Current user's wants (if logged in) for heart state
  const { data: myWants } = useWants(null);
  const addWant = useAddWant();
  const removeWant = useRemoveWant();

  const myWantsMap = useMemo(() => {
    const map = new Map<string, Want>();
    if (myWants) {
      for (const w of myWants) {
        map.set(w.wantId, w);
      }
    }
    return map;
  }, [myWants]);

  // Get the active event from the store — or just fetch all wants
  const { data: wants, isLoading: wantsLoading } = useSharedWants(shareInfo?.uid);

  // We need to figure out which eventId(s) to load releases for
  const eventIds = useMemo(() => {
    if (!wants) return [];
    return [...new Set(wants.map((w) => w.eventId))];
  }, [wants]);

  // Fetch releases for all event IDs the wants reference
  const { data: releases, isLoading: releasesLoading } = useQuery<Release[]>({
    queryKey: ['sharedReleases', eventIds],
    enabled: eventIds.length > 0,
    queryFn: async () => {
      const allReleases: Release[] = [];
      for (const eventId of eventIds) {
        const q = query(collection(db, 'releases'), where('eventId', '==', eventId));
        const snap = await getDocs(q);
        allReleases.push(...snap.docs.map((d) => ({ releaseId: d.id, ...d.data() }) as Release));
      }
      return allReleases;
    },
    staleTime: 5 * 60 * 1000,
  });

  const isLoading = shareLoading || wantsLoading || releasesLoading;

  // Build want lookup
  const wantsMap = useMemo(() => {
    const map = new Map<string, Want>();
    if (wants) {
      for (const w of wants) {
        map.set(w.wantId, w);
      }
    }
    return map;
  }, [wants]);

  // Match wants to releases, sorted by artist
  const wantedReleases = useMemo(() => {
    if (!releases || !wants || wants.length === 0) return [];

    const wantedIds = new Set(wants.map((w) => w.releaseId));
    const filtered = releases.filter((r) => wantedIds.has(r.releaseId));

    const foundReleaseIds = new Set(filtered.map((r) => r.releaseId));
    const fallbackReleases: Release[] = wants
      .filter((w) => !foundReleaseIds.has(w.releaseId))
      .map((w) => ({
        releaseId: w.releaseId,
        eventId: w.eventId,
        artist: w.artist,
        title: w.title,
        imageUrl: w.imageUrl ?? null,
        format: w.format ?? null,
        releaseType: w.releaseType ?? null,
        label: null,
        quantity: null,
        description: null,
        detailsUrl: null,
      }));

    const all = [...filtered, ...fallbackReleases];
    all.sort((a, b) => a.artist.localeCompare(b.artist));
    return all;
  }, [releases, wants]);

  const acquiredCount = useMemo(
    () => (wants ?? []).filter((w) => w.status === 'ACQUIRED').length,
    [wants]
  );
  const total = wants?.length ?? 0;

  // --- Error / not found ---
  if (!shareLoading && (shareError || !shareInfo)) {
    return (
      <div className="flex flex-col items-center justify-center py-20 text-center px-4">
        <img src={rsdLogo} alt="RSD Wants" className="h-14 mb-6" />
        <p className="text-white font-bold text-lg">List not found</p>
        <p className="text-[#B3B3B3] text-sm mt-1">
          This shared list may no longer exist or the link is invalid.
        </p>
        <Link to="/" className="text-[#E8A530] text-sm mt-4 hover:underline">
          Go to RSD Wants
        </Link>
      </div>
    );
  }

  return (
    <div className="px-4 pt-4 pb-4 space-y-4 max-w-3xl mx-auto">
      {/* Header */}
      <header className="text-center space-y-1">
        <Link to="/">
          <img src={rsdLogo} alt="RSD Wants" className="h-14 mx-auto mb-3" />
        </Link>
        {shareLoading ? (
          <Skeleton className="h-7 w-48 mx-auto bg-[#1e1e1e]" />
        ) : (
          <>
            <h1 className="text-white font-bold text-2xl leading-tight tracking-tight">
              {shareInfo?.listName}
            </h1>
            <p className="text-xs text-[#B3B3B3]">by {shareInfo?.ownerName}</p>
            {!isLoading && total > 0 && (
              <p className="text-xs text-[#B3B3B3]">
                {acquiredCount} of {total} found
              </p>
            )}
          </>
        )}
      </header>

      {/* Loading skeleton */}
      {isLoading && (
        <div className="grid-albums">
          {Array.from({ length: 6 }).map((_, i) => (
            <div key={i} className="space-y-2">
              <Skeleton className="aspect-square rounded-lg bg-[#1e1e1e]" />
              <Skeleton className="h-4 w-3/4 bg-[#1e1e1e]" />
            </div>
          ))}
        </div>
      )}

      {/* Empty state */}
      {!isLoading && total === 0 && (
        <div className="flex flex-col items-center justify-center py-20 text-center">
          <p className="text-[#B3B3B3] text-sm">This list is empty</p>
        </div>
      )}

      {/* Grid view — with drill-down + hearts */}
      {!isLoading && wantedReleases.length > 0 && (
        <div className="grid-albums">
          {wantedReleases.map((release) => {
            const ownerWant = wantsMap.get(buildWantId(release.eventId, release.releaseId));
            const myWant = myWantsMap.get(buildWantId(release.eventId, release.releaseId));
            return (
              <SharedReleaseCard
                key={release.releaseId}
                release={release}
                ownerWant={ownerWant}
                myWant={myWant}
                isAuthenticated={isAuthenticated}
                onHeartClick={(r) => {
                  if (!isAuthenticated) {
                    navigate('/auth');
                    return;
                  }
                  if (myWant) {
                    removeWant.mutate(myWant.wantId);
                  } else {
                    addWant.mutate(r);
                  }
                }}
              />
            );
          })}
        </div>
      )}

      {/* Footer CTA — only for logged-out users */}
      {!isAuthenticated && (
        <div className="text-center pt-6 pb-8 space-y-2">
          <p className="text-white font-bold text-sm">Track your own RSD wants</p>
          <p className="text-[#B3B3B3] text-xs">
            Build your list, mark what you've found, and share it with friends.
          </p>
          <Button asChild size="sm" className="mt-2">
            <Link to="/auth">Create your own list</Link>
          </Button>
        </div>
      )}
    </div>
  );
}

/** Release card for shared lists — supports drill-down and hearts */
function SharedReleaseCard({
  release,
  ownerWant,
  myWant,
  isAuthenticated,
  onHeartClick,
}: {
  release: Release;
  ownerWant?: Want;
  myWant?: Want;
  isAuthenticated: boolean;
  onHeartClick: (release: Release) => void;
}) {
  const isAcquired = ownerWant?.status === 'ACQUIRED';
  const hasMyWant = !!myWant;
  const navigate = useNavigate();

  return (
    <div className="group">
      {/* Album art — links to detail page */}
      <div
        className="block cursor-pointer"
        role="link"
        tabIndex={0}
        onClick={() => navigate(`/release/${release.releaseId}`)}
        onKeyDown={(e) => e.key === 'Enter' && navigate(`/release/${release.releaseId}`)}
      >
        <div className="aspect-square rounded-lg overflow-hidden bg-[#1e1e1e] relative">
          {release.imageUrl ? (
            <img
              src={release.imageUrl}
              alt={`${release.artist} – ${release.title}`}
              className="w-full h-full object-cover"
              loading="lazy"
            />
          ) : (
            <div className="w-full h-full flex items-center justify-center">
              <span className="text-[#333] text-3xl">♫</span>
            </div>
          )}
          {isAcquired && (
            <div className="absolute top-1.5 right-1.5 bg-black/70 rounded-full px-2 py-0.5">
              <span className="text-[10px] text-[#DEA84A] font-bold">GOT IT</span>
            </div>
          )}
        </div>
      </div>

      {/* Title + Artist + heart */}
      <div className="flex items-start gap-1.5 mt-1.5">
        <div className="flex-1 min-w-0">
          <p className="text-sm text-white font-bold leading-tight truncate">{release.title}</p>
          <p className="text-xs text-[#B3B3B3] leading-tight truncate">{release.artist}</p>
        </div>

        <button
          type="button"
          className="flex-shrink-0 p-0.5"
          onClick={() => onHeartClick(release)}
          aria-label={
            hasMyWant ? `Remove ${release.title} from wants` : `Add ${release.title} to wants`
          }
        >
          {isAuthenticated && hasMyWant ? (
            <svg
              width="20"
              height="19"
              viewBox="0 0 20 19"
              fill="currentColor"
              xmlns="http://www.w3.org/2000/svg"
              className="text-white h-5 w-5"
            >
              <path d="M18.2897 1.78491C17.2653 0.71139 15.89 0.0769175 14.4363 0.00731183C12.9827 -0.0622939 11.557 0.438051 10.4419 1.40913C10.3136 1.50968 10.1591 1.56766 9.99857 1.57548C9.83707 1.57059 9.68127 1.51231 9.55384 1.40913C8.43924 0.437498 7.01361 -0.0632202 5.56003 0.00639588C4.10645 0.0760119 2.73123 0.710871 1.70744 1.78491C1.16436 2.34595 0.733815 3.0133 0.440735 3.74834C0.147654 4.48339 -0.00214049 5.27151 2.31073e-05 6.0671C2.31073e-05 7.68461 0.606343 9.20558 1.67026 10.3077L8.05092 18.0655C8.29135 18.3585 8.59014 18.5938 8.92665 18.7551C9.26316 18.9164 9.62937 19 10 19C10.3706 19 10.7368 18.9164 11.0734 18.7551C11.4099 18.5938 11.7087 18.3585 11.9491 18.0655L18.2911 10.3493C18.8343 9.78827 19.2651 9.12095 19.5584 8.38592C19.8517 7.6509 20.0018 6.86276 20 6.0671C20.0012 5.27139 19.8507 4.48331 19.5572 3.74833C19.2636 3.01336 18.8329 2.34605 18.2897 1.78491Z" />
            </svg>
          ) : (
            <img src={heartIcon} alt="" className="h-5 w-5" style={{ opacity: 0.4 }} />
          )}
        </button>
      </div>
    </div>
  );
}
