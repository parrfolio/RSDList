...
For specific token values (colors, fonts, spacing, shadows), the **design-system** skill is the single source of truth. This skill defines design PHILOSOPHY and QUALITY standards — not specific values.
...
- Building web applications with React/TanStack Start/similar frameworks
...
1. Search shadcn/ui first (https://ui.shadcn.com) — the ONLY approved component library
2. Build custom only when no suitable shadcn/ui component exists
...
- `foundation/typography.md` - Typographic principles, scales, loading
...
- **Typography**: Defer to design-system for font choices; 14px minimum for code
...
**shadcn/ui** (https://ui.shadcn.com) is the ONLY approved component library. Do not use daisyUI, HeroUI, or alternatives.

- Excellent accessibility defaults via Radix UI primitives
- Tailwind-based, fully customizable
- Copy-paste into project for full ownership
- Consistent with project design tokens

See `reference/design-systems.md` for usage patterns and customization guidance.

## Typography Principles

For project typography, defer to the **design-system** skill for specific font choices and token values. This skill focuses on typographic PRINCIPLES — not specific font selections.

**Apply these principles regardless of font:**

- Establish clear visual hierarchy through weight contrast and size scales
- Maintain consistent rhythm with a modular type scale
- Use a single sans-serif for UI and a single monospace for code (14px minimum)
- Never mix multiple sans-serif or multiple monospace fonts
- Optimize readability: appropriate line height, letter spacing, and measure

See `foundation/typography.md` for detailed typographic guidance.
...
└── reference/
    ├── design-systems.md (shadcn/ui usage and customization)
