...
| `agents`      | No       | Array of agent names that should consume this skill (see below)                     |

### `agents` (recommended)

An array of agent names that should consume this skill. When conductor.powder orchestrates work, she scans skill frontmatter for this field to determine which skills to inject into each subagent's context.
...
- Use exact agent names as they appear in `.github/agents/*.agent.md` filenames (without the `.agent.md` extension)
- Only list agents whose work directly benefits from this skill's knowledge
- Common agent targets:
  - `frontend.implementation` — for frontend/UI skills (animations, components, styling)
  - `design.ux-engineer` — for UX/design skills (research, personas, design systems)
  - `frontend.design-system` — for design system/Figma skills
  - `architecture.context` — for research methodology skills
  - `engineering.implementation` — for backend implementation skills
  - `conductor.powder` — for orchestration/discovery skills
- A skill can target multiple agents
- If omitted, the skill will not be automatically injected into any subagent's context (it will still be available at the top-level conversation via VS Code's skill discovery)
